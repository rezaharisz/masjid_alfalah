 ...
 * Page class
 */
class PengurusMasjidEdit extends PengurusMasjid
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PengurusMasjidEdit";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "pengurusmasjidview") {
                        $row["view"] = "1";
                    }
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("ID_PENGURUS") ?? Key(0) ?? Route(2)) !== null) {
                $this->ID_PENGURUS->setQueryStringValue($keyValue);
                $this->ID_PENGURUS->setOldValue($this->ID_PENGURUS->QueryStringValue);
            } elseif (Post("ID_PENGURUS") !== null) {
                $this->ID_PENGURUS->setFormValue(Post("ID_PENGURUS"));
                $this->ID_PENGURUS->setOldValue($this->ID_PENGURUS->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("ID_PENGURUS") ?? Route("ID_PENGURUS")) !== null) {
                    $this->ID_PENGURUS->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->ID_PENGURUS->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("pengurusmasjidlist"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "pengurusmasjidlist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'ID_PENGURUS' first before field var 'x_ID_PENGURUS'
        $val = $CurrentForm->hasValue("ID_PENGURUS") ? $CurrentForm->getValue("ID_PENGURUS") : $CurrentForm->getValue("x_ID_PENGURUS");
        if (!$this->ID_PENGURUS->IsDetailKey) {
            $this->ID_PENGURUS->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->ID_PENGURUS->CurrentValue = $this->ID_PENGURUS->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // ID_PENGURUS
            $this->ID_PENGURUS->EditAttrs["class"] = "form-control";
            $this->ID_PENGURUS->EditCustomAttributes = "";
            $this->ID_PENGURUS->EditValue = $this->ID_PENGURUS->CurrentValue;
            $this->ID_PENGURUS->ViewCustomAttributes = "";
    ...
    protected function validateForm()
    {
        ...
        if ($this->ID_PENGURUS->Required) {
            if (!$this->ID_PENGURUS->IsDetailKey && EmptyValue($this->ID_PENGURUS->FormValue)) {
                $this->ID_PENGURUS->addErrorMessage(str_replace("%s", $this->ID_PENGURUS->caption(), $this->ID_PENGURUS->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("pengurusmasjidlist"), "", $this->TableVar, true);
        $pageId = "edit";

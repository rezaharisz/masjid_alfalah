 ...
 * Page class
 */
class PerlengkapanIbadahAdd extends PerlengkapanIbadah
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PerlengkapanIbadahAdd";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("NAMA_PERLENGKAPAN_IBADAH") ?? Route("NAMA_PERLENGKAPAN_IBADAH")) !== null) {
                $this->NAMA_PERLENGKAPAN_IBADAH->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("perlengkapanibadahlist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "perlengkapanibadahlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "perlengkapanibadahview") {
    ...
    {
        $this->NAMA_PERLENGKAPAN_IBADAH->CurrentValue = null;
        $this->NAMA_PERLENGKAPAN_IBADAH->OldValue = $this->NAMA_PERLENGKAPAN_IBADAH->CurrentValue;
        $this->JUMLAH->CurrentValue = null;
        $this->JUMLAH->OldValue = $this->JUMLAH->CurrentValue;
        $this->KONDISI->CurrentValue = null;
        $this->KONDISI->OldValue = $this->KONDISI->CurrentValue;
        $this->TANGGAL_PEMBELIAN->CurrentValue = null;
        $this->TANGGAL_PEMBELIAN->OldValue = $this->TANGGAL_PEMBELIAN->CurrentValue;
        $this->TANGGAL_BELI_GANTI_BARU->CurrentValue = null;
        $this->TANGGAL_BELI_GANTI_BARU->OldValue = $this->TANGGAL_BELI_GANTI_BARU->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'NAMA_PERLENGKAPAN_IBADAH' first before field var 'x_NAMA_PERLENGKAPAN_IBADAH'
        $val = $CurrentForm->hasValue("NAMA_PERLENGKAPAN_IBADAH") ? $CurrentForm->getValue("NAMA_PERLENGKAPAN_IBADAH") : $CurrentForm->getValue("x_NAMA_PERLENGKAPAN_IBADAH");
        if (!$this->NAMA_PERLENGKAPAN_IBADAH->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->NAMA_PERLENGKAPAN_IBADAH->Visible = false; // Disable update for API request
            } else {
                $this->NAMA_PERLENGKAPAN_IBADAH->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->NAMA_PERLENGKAPAN_IBADAH->CurrentValue = $this->NAMA_PERLENGKAPAN_IBADAH->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['NAMA_PERLENGKAPAN_IBADAH'] = $this->NAMA_PERLENGKAPAN_IBADAH->CurrentValue;
        $row['JUMLAH'] = $this->JUMLAH->CurrentValue;
        $row['KONDISI'] = $this->KONDISI->CurrentValue;
        $row['TANGGAL_PEMBELIAN'] = $this->TANGGAL_PEMBELIAN->CurrentValue;
        $row['TANGGAL_BELI_GANTI_BARU'] = $this->TANGGAL_BELI_GANTI_BARU->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->NAMA_PERLENGKAPAN_IBADAH->EditValue = HtmlEncode($this->NAMA_PERLENGKAPAN_IBADAH->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->NAMA_PERLENGKAPAN_IBADAH->Required) {
            if (!$this->NAMA_PERLENGKAPAN_IBADAH->IsDetailKey && EmptyValue($this->NAMA_PERLENGKAPAN_IBADAH->FormValue)) {
                $this->NAMA_PERLENGKAPAN_IBADAH->addErrorMessage(str_replace("%s", $this->NAMA_PERLENGKAPAN_IBADAH->caption(), $this->NAMA_PERLENGKAPAN_IBADAH->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // NAMA_PERLENGKAPAN_IBADAH
        $this->NAMA_PERLENGKAPAN_IBADAH->setDbValueDef($rsnew, $this->NAMA_PERLENGKAPAN_IBADAH->CurrentValue, "", false);

        // JUMLAH
        $this->JUMLAH->setDbValueDef($rsnew, $this->JUMLAH->CurrentValue, 0, false);

        // KONDISI
        $this->KONDISI->setDbValueDef($rsnew, $this->KONDISI->CurrentValue, "", false);

        // TANGGAL_PEMBELIAN
        $this->TANGGAL_PEMBELIAN->setDbValueDef($rsnew, UnFormatDateTime($this->TANGGAL_PEMBELIAN->CurrentValue, 7), CurrentDate(), false);

        // TANGGAL_BELI_GANTI_BARU
        $this->TANGGAL_BELI_GANTI_BARU->setDbValueDef($rsnew, UnFormatDateTime($this->TANGGAL_BELI_GANTI_BARU->CurrentValue, 7), CurrentDate(), false);

        // Call Row Inserting event
        $insertRow = $this->rowInserting($rsold, $rsnew);

        // Check if key value entered
        if ($insertRow && $this->ValidateKey && strval($rsnew['NAMA_PERLENGKAPAN_IBADAH']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("perlengkapanibadahlist"), "", $this->TableVar, true);

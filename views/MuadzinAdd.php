...
loadjs.ready("head", function () {
    ...
    fmuadzinadd = currentForm = new ew.Form("fmuadzinadd", "add");

    // Add fields
    var fields = ew.vars.tables.muadzin.fields;
    fmuadzinadd.addFields([
    ...
    // Form_CustomValidate
    fmuadzinadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmuadzinadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmuadzinadd.lists.WAKTU_ADZAN = <?= $Page->WAKTU_ADZAN->toClientList($Page) ?>;
    loadjs.done("fmuadzinadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmuadzinadd" id="fmuadzinadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fmuadzinadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fmuadzinadd", "x_TANGGAL_ADZAN", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>

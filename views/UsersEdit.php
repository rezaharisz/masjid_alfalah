...
loadjs.ready("head", function () {
    ...
    fusersedit.addFields([
        ...
        ["userlevelid", [fields.userlevelid.required ? ew.Validators.required(fields.userlevelid.caption) : null], fields.userlevelid.isInvalid]
    ]);
    ...
    fusersedit.lists.userlevelid = <?= $Page->userlevelid->toClientList($Page) ?>;
...
<input type="<?= $Page->name->getInputTextType() ?>" data-table="users" data-field="x_name" name="x_name" id="x_name" size="30" maxlength="50" placeholder="<?= HtmlEncode($Page->name->getPlaceHolder()) ?>" value="<?= $Page->name->EditValue ?>"<?= $Page->name->editAttributes() ?> aria-describedby="x_name_help">
...
<input type="<?= $Page->_password->getInputTextType() ?>" data-table="users" data-field="x__password" name="x__password" id="x__password" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->_password->getPlaceHolder()) ?>" value="<?= $Page->_password->EditValue ?>"<?= $Page->_password->editAttributes() ?> aria-describedby="x__password_help">
...
<?php if ($Page->userlevelid->Visible) { // userlevelid ?>
    <div id="r_userlevelid" class="form-group row">
        <label id="elh_users_userlevelid" for="x_userlevelid" class="<?= $Page->LeftColumnClass ?>"><?= $Page->userlevelid->caption() ?><?= $Page->userlevelid->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_users_userlevelid">
    <select
        ...
        data-select2-id="users_x_userlevelid"
        data-table="users"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='users_x_userlevelid']"),
        options = { name: "x_userlevelid", selectId: "users_x_userlevelid", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.users.fields.userlevelid.selectOptions);
